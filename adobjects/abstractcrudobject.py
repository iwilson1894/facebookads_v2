...
from facebookads_v2.exceptions import (
    FacebookBadObjectError,
)
from facebookads_v2.api import (
    FacebookAdsApi,
    Cursor,
    FacebookRequest,
)
...
class AbstractCrudObject(AbstractObject):
    ...
    def iterate_edge_async(self, target_objects_class, fields=None,
                           params=None, async_var=False, include_summary=True,
                           endpoint=None):
        from facebookads_v2.adobjects.adreportrun import AdReportRun
        """
        Behaves as iterate_edge(...) if parameter async_var if False
        (Default value)
        If async_var is True:
        ...
        >>> job = object.iterate_edge_async(
                TargetClass, fields, params, async_var=True)
        >>> time.sleep(10)
        >>> job.remote_read()
        >>> if job:
        ...
        # To force an async_var response from an edge, do a POST instead of GET.
        # The response comes in the format of an AsyncJob which
        # indicates the progress of the async_var request.
        if endpoint is None:
